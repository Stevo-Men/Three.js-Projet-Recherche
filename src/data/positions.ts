export const POSITIONS = [
    { side: 'center', top: '10%' }, // 0 Plan
    { side: 'left', top: '12%' },  // 1 Plugin Era
    { side: 'left', top: '18%' },  // 1  WebGL arrival
    { side: 'left', top: '15%' },  // 2  Pipeline
    { side: 'left', top: '52%' },  // 3  Transition
    { side: 'left', top: '15%' },  // 4  WebGPU
    { side: 'left', top: '50%' },  // 5  Three.js
    { side: 'left', top: '8%', codePos: { side: 'right', top: '10%' } },  // 7  Architecture (code)
    { side: 'left', top: '10%', codePos: { side: 'center', top: '15%' } },  // 8  Objets 3D (code)
    { side: 'right', top: '22%', codePos: { side: 'left', top: '35%' } },  // 9  Matériaux (code)
    { side: 'left', top: '16%', codePos: { side: 'right', top: '25%' } },  // 10  Lumières (code)
    { side: 'right', top: '10%' },  // 10  Alternatives
    { side: 'left', top: '14%' },   // Cas d'utilisation
    { side: 'right', top: '18%' },  // Limites actuelles
    { side: 'left', top: '16%' },   // Technologies émergentes
];
